---
name: fitness
description: Design, progress, and adapt fitness programs (strength, cardio, endurance) based on quantified rules. Trigger when building a training plan, computing loads, troubleshooting plateaus, adjusting for missed sessions, or interpreting wearable readiness data. Bypass this skill for rep-by-rep logging or meal-level nutrition.
metadata:
  version: "1.0.0"
  openclaw: '{"emoji":"💪","requires":{"config":["<state_root>/fitness/"]}}'
  related-skills: '{"calories":"Deficit or surplus math behind fat-loss and muscle-gain phases.","gym":"Rep-by-rep workout logging and in-session routine execution.","running":"Run-specific pacing, race prep, and running injury prevention.","sleep":"Readiness checks when sleep is the limiting recovery input."}'
---

Training logs, preferences, and derived baselines (e1RMs, resting HR, attendance rate) persist in `<state_root>/fitness/` as plain markdown (see `references/setup.md` on first use, `references/memory-template.md` for file formats). If you have data at an old location (`~/fitness/` or `~/clawic/fitness/`), move it to `<state_root>/fitness/`, and say in one line that you moved it and from where. Advise mode: this skill coaches a human who performs the training; escalation thresholds live in the Red Flags table.

## State location

- `<state_root>/fitness/` (First choice, fallback if needed)

## Configuration

User-dependent variables. Defaults apply until the user states a preference; store them in `<state_root>/fitness/config.yaml`.

| Variable | Type | Default | Effect |
|---|---|---|---|
| units | metric \| imperial | metric | All loads, distances, and body-mass numbers render in this system; formulas unchanged |
| training_days | number (2-6) | 3 | Selects the split (`references/program-design.md`) and divides weekly set targets into sessions |
| session_minutes | number (20-120) | 60 | Caps sets per session; under 40 switches to the time-crunched templates in `references/minimal-equipment.md` |
| equipment | full-gym \| home-rack \| dumbbells \| bodyweight | full-gym | Filters exercise selection and drives the substitution table in `references/program-design.md` |
| primary_goal | strength \| muscle \| endurance \| fat-loss \| general | general | Picks the Quick Reference row used as the program base |
| age | number (years) | none | Feeds the Tanaka HRmax estimate (Rule 5) when `measured_hrmax` is unset; falls back to optional shared profile age when available |
...
- **data sources** — which wearable or app supplies HR, sleep, and steps — sets how `references/tracking.md` interprets their numbers
...
| No training history on file | Baseline and starting-load protocol in `references/assessment.md` before prescribing loads |
| Returning after 2+ weeks off | Apply the layoff discount (Rule 6); full gap-length table in `references/comeback.md` |
| Lift stalled 3 sessions | Run the diagnostic order in Progression and Plateaus; playbooks in `references/progression.md` |
| Scale flat 2+ weeks | Weekly-average check in `references/body-composition.md` before touching calories or cardio |
| Pain reported | Check Red Flags first, then the DOMS-vs-injury test in Recovery and Readiness |
| No gym, hotel room, 25 minutes | Substitutions and time-crunched templates in `references/minimal-equipment.md` |
| Wearable flags "low readiness" | Validate against resting HR + sleep + log (`references/tracking.md`) before changing the plan |
...
Depth on demand: `references/assessment.md` baselines, testing, starting loads · `references/program-design.md` splits, templates, substitutions · `references/progression.md` progression models and plateau playbooks · `references/cardio.md` zones, intervals, concurrent training · `references/recovery.md` readiness, deloads, overtraining · `references/comeback.md` layoffs, illness, injury return · `references/body-composition.md` fat-loss and muscle-gain phases · `references/minimal-equipment.md` home, travel, time-crunched · `references/tracking.md` logs and wearables.
...
5. **Cardio: the 80% easy share is the load-bearing number.** Easy means full sentences are speakable. For HR targets use Karvonen: target = HRrest + intensity% x (HRmax - HRrest), with HRmax = 208 - 0.7 x age (Tanaka; the 220 - age formula drifts 10+ bpm off past age 40). Read age from `age` (config, else optional shared profile age when available); a measured max (`measured_hrmax`) overrides the estimate and needs no age.
6. **Layoff discount by gap length.** A single missed session (gap under one week) costs nothing: skip it, resume as planned, keeping the sequence linear without stacking. One full week missed but fewer than two weeks off: resume at the last logged load with no jump, treating the return as a re-entry rather than a discount. Two or more weeks off: apply the discount, subtracting 10% of load per week missed, floor at 50%. Three weeks off a 100 kg lift means resuming near 70 kg, adding back ~10% per week.
...
Full templates, exercise selection, and the equipment substitution table: `references/program-design.md`.
...
Per-symptom plateau playbooks and periodization beyond double progression: `references/progression.md`.

## Recovery and Readiness

- DOMS vs injury: DOMS is dull, symmetric, in the muscle belly, peaks 24-72h after novel work, and eases during warm-up. Sharp, one-sided, at a joint or tendon, present at rest, or worsening through warm-up = treat as injury and suspend loading that pattern (see Red Flags).
...
Deload execution, the overtraining ladder, and stress-adjusted training: `references/recovery.md`.

## Red Flags

| Signal (observable) | Suspicion | Action |
|---|---|---|
| Chest pain or pressure during exertion, radiating to arm or jaw | Cardiac event | Suspend immediately, emergency services |
| Fainting or near-fainting during or right after exercise | Arrhythmia or cardiac cause | Suspend training, clinician this week, no intensity until cleared |
| Cola-colored urine plus severe swelling after an extreme session | Rhabdomyolysis | Emergency care the same day |
| Sharp joint pain with swelling, or pain that alters gait | Structural injury | Suspend loading that pattern; clinician if not improving in 5-7 days |
| Numbness, tingling, or weakness down a limb | Nerve involvement | Suspend the provoking exercise, clinician |
...
| Chasing soreness as proof of a good session | DOMS tracks novelty, not growth; a stable program ceases to induce soreness while still working | Judge by log trend (`references/tracking.md`), not by ache |
...
- **How much periodization non-athletes need.** Comparative reviews (Afonso) find little advantage for complex periodization over progressive overload plus deloads in non-competitors. Default: double progression with Rule 7 deloads; block periodization only when peaking for a dated event (`references/progression.md`).
- **Cardio while building muscle.** The interference camp minimizes it; the health camp keeps it. Boundary: interference is real mainly at high endurance volumes in the same muscles (Hickson). Default: keep the ACSM 150-min floor, low-impact, placed by the ordering rules in `references/cardio.md`.
