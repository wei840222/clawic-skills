...
Configuration and memory are stored in a local workspace directory. The skill looks for state in the following candidate locations, prioritizing the first one found:

1. `<state_root>/`

If no directory exists, it will create `<state_root>/` and use it.


## References
All deep knowledge, patterns, and configuration templates are stored in `references/`. Load the specific files below when you encounter these scenarios:

| File | When to load |
|---|---|
| `references/layout.md` | When building grids, flexbox, or diagnosing overflows. |
| `references/architecture.md` | When deciding on component boundaries, layers, or specificity constraints. |
| `references/responsive.md` | When working with viewports, container queries, or fluid typography. |
| `references/theming.md` | When dealing with dark mode, color schemes, or tokens. |
| `references/performance.md` | When debugging CLS, stuttering animations, or repaint costs. |
| `references/compatibility.md` | When a feature works in one browser but not another. |
| `references/selectors.md` | When using `:has()`, `@layer`, or complex specificity. |
...
|---|---|---|---|
| authoring_mode | plain-css \| sass \| tailwind \| css-in-js | plain-css | Syntax of every emitted example, and which advice in `references/architecture.md` applies (layers and nesting vs utility ordering vs zero-runtime extraction) |
| browser_support | evergreen \| widely-available \| legacy | evergreen | Gates which features ship bare vs behind `@supports`: `legacy` also adds vendor-prefixed fallbacks and blocks the Chromium-first features flagged in `references/compatibility.md` |
| naming_convention | bem \| utility \| css-modules \| none | none | Class names in examples and the specificity ceiling enforced in reviews (`references/architecture.md` — Keeping It Clean) |
| rem_base | number px (10-16) | 16 | Every px→rem conversion, including the fluid-type derivation in `references/responsive.md` (rem term = px ÷ rem_base) |
...
| The rule is written but nothing changes | `references/debugging.md` — symptom→cause chains, starting with "is it even matching?" |
| Breaks with real content, sticky dead, footer floats, margin leaks | `references/layout.md` |
| Component must adapt to its container; fluid type; mobile viewport bugs | `references/responsive.md` |
| Specificity fight, `@layer`, `:has()`, custom-property gotchas | `references/selectors.md` |
| Jank, layout shift, slow paint, font flash | `references/performance.md` |
| Transition never fires, enter/exit animation, view transitions, reduced motion | `references/animations.md` |
| Line-height inheritance, measure, variable fonts, hyphenation, tabular numbers | `references/typography.md` |
| oklch vs hsl, `color-mix()`, gradient banding, wide gamut | `references/color.md` |
| Dark mode, design tokens, multi-brand, theme flash on load | `references/theming.md` |
| Inputs, selects, checkboxes, validation states, autofill styling | `references/forms.md` |
| Modal behind the header, dropdown clipped, tooltip placement | `references/overlays.md` |
| Scroll snap, custom scrollbars, anchor link lands under the sticky header | `references/scrolling.md` |
| Data table: sticky header, responsive behavior, column sizing | `references/tables.md` |
| Shadows, filters, masks, `clip-path`, blend modes, 3D transforms | `references/effects.md` |
| Starting a stylesheet: what goes in the reset and base layers, in what order | `references/reset.md` |
| Sheet organization, layer strategy, nesting, Sass migration, shadow DOM | `references/architecture.md` |
| Works in Chrome, breaks in Safari or Firefox; HTML email | `references/compatibility.md` |
| Print or PDF output wrong | `references/print.md` |
| RTL mirroring, CJK line breaking, logical properties, text expansion | `references/internationalization.md` |
| Anything else CSS | Core Rules below, then `references/debugging.md` to name the mechanism |
...
4. Never bare viewport units for text. `font-size: clamp(1rem, 0.77rem + 0.91vw, 1.5rem)` — the rem term is what keeps browser zoom and user font-size working; pure-vw text fails WCAG 1.4.4 (resize to 200%). Derivation of the numbers: `references/responsive.md`.
...
6. `!important` in component code is a debt marker. Order wars belong in `@layer` — unlayered author styles beat all layered ones regardless of specificity (`references/selectors.md`).
7. Overlays belong in the top layer, not high in the z-index scale. `<dialog>.showModal()` and `popover` escape every stacking context and every `overflow: hidden` ancestor; a z-index arms race means the wrong mechanism is in use (`references/overlays.md`).
...
- `gap` never collapses; margins collapse (vertical, block layout only, including parent-child bleed-through). Prefer gap and treat margin collapse as legacy behavior to route around (`references/layout.md`).
...
Compatibility floor: everything here is in all three engines unless marked; version-sensitive items are dated in `references/compatibility.md`.

- `:has()` — parent and previous-sibling selection; kills a whole class of state-mirroring JS (`references/selectors.md` for patterns and cost).
...
- `light-dark()` + `color-scheme` — one declaration per token instead of a duplicated dark block (all engines since 2024; `references/theming.md`).
...
- `scroll-snap-type` + `scroll-snap-align` — carousels without JS (`references/scrolling.md`).
- `aspect-ratio` — reserve media space before load (layout-shift numbers: `references/performance.md`).
- `accent-color` — form controls on brand without rebuilding them (`references/forms.md`).
- `@scope` and native nesting — component boundaries without naming conventions; specificity traps in `references/architecture.md`.
...
- Anchor positioning (`anchor-name`, `position-area`) — tethered popovers without a positioning library; still needs a fallback, see `references/overlays.md`.
...
- Style `:focus-visible`; always provide a visible replacement when using `outline: none` in the same rule.
...
- Dark mode checked if the project themes, and RTL checked if `references/internationalization.md` is in scope?
...
- Selector performance: the old guard writes selectors for right-to-left matching cost; modern engines bucket by rightmost simple selector, making it negligible. Boundary: only act on a DevTools trace showing Style/Recalculate cost — usually `:has()` or universal selectors on large, frequently-mutating DOMs (`references/performance.md`).
- Utility-first vs handwritten CSS: utilities win on team consistency and dead-code elimination; handwritten wins for animation-heavy and design-led work. Boundary: follow whichever the codebase already uses; ensure a consistent styling system is maintained within each component.
